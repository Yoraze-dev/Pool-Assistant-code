...
class PoolAssistantSupabaseUser extends BaseAuthUser {
  PoolAssistantSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> poolAssistantSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PoolAssistantSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
